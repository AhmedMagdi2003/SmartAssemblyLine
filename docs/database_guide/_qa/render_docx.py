...
def _respect_explicit_tempdir() -> None:
    """Honor explicit temp overrides on non-macOS platforms too.

    In sandboxed environments the process environment can point TEMP/TMP/TMPDIR
    at a writable workspace path, but `tempfile` may already have resolved the
    system temp directory before rendering begins. Resetting `tempfile.tempdir`
    here makes the renderer follow the explicit writable override.
    """
...
@contextmanager
def _managed_lo_workdirs():
    """Use stable writable workdirs when the caller provides a temp base.

    Some sandboxed Windows environments create temp folders that become
    inaccessible to follow-up cleanup. Allow a stable workspace path to be
    injected through `RENDER_DOCX_BASE_TMP` so rendering can avoid those temp
    folder permission problems entirely.
    """

    base_tmp = os.environ.get("RENDER_DOCX_BASE_TMP", "").strip()
    if base_tmp:
        user_profile = join(base_tmp, "soffice_profile")
        convert_tmp_dir = join(base_tmp, "soffice_convert")
        shutil.rmtree(user_profile, ignore_errors=True)
        shutil.rmtree(convert_tmp_dir, ignore_errors=True)
        makedirs(user_profile, exist_ok=True)
        makedirs(convert_tmp_dir, exist_ok=True)
        yield user_profile, convert_tmp_dir
        return
...
def _prepend_bundled_runtime_bin() -> None:
    ...
    if os.path.basename(python_root) == "python":
        ...
        if os.path.basename(dependencies_root) == "dependencies":
            runtime_bins.append(os.path.join(dependencies_root, "bin"))

    current = os.path.dirname(os.path.realpath(__file__))
    while current and current != os.path.dirname(current):
        if os.path.basename(current) == "codex-primary-runtime":
            runtime_bins.append(os.path.join(current, "dependencies", "bin"))
            break
    ...
    for runtime_bin in runtime_bins:
        if not os.path.isdir(runtime_bin):
            continue
        if runtime_bin in path_entries:
            return

        os.environ["PATH"] = os.pathsep.join(
            [runtime_bin, *[entry for entry in path_entries if entry]]
        )
        return
...
def calc_dpi_via_pdf(input_path: str, max_w_px: int, max_h_px: int, verbose: bool) -> int:
    ...
    _default_macos_tmpdir_for_soffice()
    _respect_explicit_tempdir()
    with _managed_lo_workdirs() as (user_profile, convert_tmp_dir):
